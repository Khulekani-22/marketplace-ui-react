// src/components/DashboardWithServicesLite.tsx
...
const containsKeyword = (value: any): boolean => {
  if (!value) return false;
  const lower = value.toString().toLowerCase();
  return MENTORSHIP_KEYWORDS.some((kw) => lower.includes(kw));
};

const isMentorshipCandidate = (service: Partial<Service> = {}): boolean => {
  ...
  if (Array.isArray(service.tags) && service.tags.some((tag: any) => containsKeyword(tag))) return true;
  if (service.requiresScheduling || service.bookingRequired || service.bookingConfig) return true;
  return false;
};

const resolveListingType = (service: Partial<Service> = {}): string => {
...
const normalize = (service: Partial<Service> = {}): Service => {
...
const isApproved = (s: Service): boolean => s.status === "approved";
...
function isServiceListing(service: Partial<Service> = {}): boolean {
...
const pickFresher = (a: Partial<Service> = {}, b: Partial<Service> = {}): Partial<Service> => {
...
const mergeLists = (currentList: Service[] = [], baseList: Service[] = [], liveList: Service[] = []): Service[] => {
  const map = new Map();
...
const TrendingNFTsOne = ({
  ...
  const [reviews, setReviews] = useState<Record<string | number, { rating?: number; comment?: string }>>({}); // serviceId -> { rating, comment }
  const [modal, setModal] = useState<{ open: boolean; id: any; showAll: boolean; page: number }>({ open: false, id: null, showAll: false, page: 0 });
  const [detailsModal, setDetailsModal] = useState<{ open: boolean; id: any }>({ open: false, id: null });
  const [toast, setToast] = useState("");
  const [toastType, setToastType] = useState<"success" | "danger">("success"); // success | danger
  const [busy, setBusy] = useState(false);
  const [subs, setSubs] = useState(() => new Set<string | number>()); // serviceId set
  const [bookings, setBookings] = useState<Record<string | number, { date: string; slot: string }>>({}); // serviceId -> { date, slot }
  const [unifiedModal, setUnifiedModal] = useState<{ open: boolean; id: any; date: string; slot: string; payment: string; voucherCode: string; sponsoredGroup: string; error: string }>({ open: false, id: null, date: '', slot: '', payment: 'credits', voucherCode: '', sponsoredGroup: '', error: '' });
  ...
  const loadServices = useCallback(
    async ({ background = false, forceRefresh = false } = {}) => {
      ...
      const fallbackApproved = safeArray((appData as any)?.services).map(normalize).filter(isApproved);

      const applyResult = (remoteApproved: Service[], fallbackFlag = false) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const bookingMap: Record<string, { date: string; slot: string }> = {};
        const set = new Set(
          items
            .filter((x: any) => !x.canceledAt)
            .map((x: any) => {
  ...
  async function toggleSubscribe(serviceId: any, options: any = {}) {
    ...
    try {
      ...
      if (!bookable && price > 0) {
        ...
        setTimeout(() => (refresh() as any)?.catch?.(() => void 0), 100);
        setToastType('success');
  ...
  // --- Reviews helpers ---
  function setField(id: any, k: string, v: any) {
    setReviews((prev) => ({ ...prev, [id]: { ...(prev[id] || {}), [k]: v } }));
  }
  function openReview(id: any) {
  ...
  function setUnifiedField(field: string, value: any) {
  ...
  function setStar(id: any, n: number) { setField(id, "rating", n); }
  ...
  function sortReviews(list: any[]) {
    const arr = Array.isArray(list) ? list.slice() : [];
    arr.sort((a: any,b: any) => (Date.parse(b?.createdAt||"")||0) - (Date.parse(a?.createdAt||"")||0));
    return arr;
  }
  const pageSize = 5;
  ...
  async function submitReview(id: any) {
        ...
        <div className="alert alert-primary d-flex flex-wrap align-items-center justify-content-between mb-3" role="status">
          ...
          <div className="d-flex gap-2 mt-2 mt-md-0">
            ...
            <button
              ...
              onClick={() => (refresh() as any)?.catch?.(() => void 0)}
                  ...
                  <div className="mb-3">
                    {[1,2,3,4,5].map((n) => (
                      <button key={n} type="button" onClick={() => setStar(svc.id, n)} className="btn btn-link p-0 me-1" aria-label={`Rate ${n} stars`}>
                        <span style={{ fontSize: 24, color: (svc.id && reviews[svc.id]?.rating || 0) >= n ? "#f5a623" : "#ccc" }}>★</span>
                      </button>
                    ))}
                  </div>
                  <div className="mb-3">
                    <textarea className="form-control" rows={3} placeholder="Write a quick comment (optional)" value={svc.id ? reviews[svc.id]?.comment || '' : ''} onChange={(e) => setField(svc.id, 'comment', e.target.value)} />
                  </div>
                  <button className="btn btn-primary" disabled={busy || Number(svc.id ? reviews[svc.id]?.rating || 0 : 0) < 1} onClick={() => submitReview(svc.id)}>
