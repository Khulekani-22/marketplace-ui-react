...
router.post("/", firebaseAuthRequired, async (req, res, next) => {
  try {
    ...
    const services = Array.isArray(data.services) ? data.services : [];
    const nextEntry = { ...parsed, id, tenantId };

    services.push(nextEntry);
    data.services = services;
    await saveData(data);

    const created = data.services.find((s) => s.id === id) || nextEntry;
...
router.put("/:id", firebaseAuthRequired, async (req, res, next) => {
  try {
    ...
    services[idx] = { ...services[idx], ...parsed };
    data.services = services;
    await saveData(data);
...
router.post("/:id/reviews", async (req, res, next) => {
  try {
    ...
    if (idx === -1 && id && id !== "undefined") {
      idx = services.findIndex((s) => String(s.id) === id);
    }
    ...
    const emailLc = (authorEmail || "").toLowerCase();
    const prevIdx = reviews.findIndex((rv) => (rv.authorEmail || "").toLowerCase() === emailLc && emailLc);
    const reviewObj = {
      id: uuidv4(),
      rating: r,
      comment,
    ...
    const priorCount = Number(service.reviewCount || (Array.isArray(service.reviews) ? service.reviews.length : 0) || 0);
    const priorAvg = Number(service.rating || 0);
    ...
    if (prevIdx >= 0) {
      ...
      newAvg = priorCount > 0 ? (priorAvg * priorCount + r) / newCount : r;
    }
    ...
    res.status(201).json(services[idx]);
  } catch (e) {
    next(e);
  }
});
